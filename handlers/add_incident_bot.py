    ...
    try:
        ...
        if result_search: 
            if len(result_search) <= 2:
                ...
                from_req.form_story_in_html(db_list=result_search, search_title="Поиск по дате", userid=message.from_user.id)
...
async def search_date_insert(message: types.Message, state=FSMContext):
    ...
    if result_search:
        if len(result_search) <= 2:
            ...
            from_req.form_story_in_html(db_list=result_search, search_title="Поиск по адресу", userid=message.from_user.id)
...
async def sort_yes(callback: CallbackQuery, state=FSMContext):
    ...
    if len(result_search) <= 2:
        ...
        from_req.form_story_in_html(db_list=result_search, search_title="Раскрытые преступления", userid=callback.from_user.id)
...
async def sort_none(callback: CallbackQuery, state=FSMContext):
    ...
    if len(result_search) <= 2:
        ...
        from_req.form_story_in_html(db_list=result_search, search_title="Нераскрытые преступления", userid=callback.from_user.id)
...
# 5.1 Поиск по ID
@dp.callback_query_handler(text="search_id", state=Incidents.incident)
async def search_date(callback: CallbackQuery, state=FSMContext):
    await callback.message.edit_text(text="Введите адрес:")
...
@dp.callback_query_handler(text="all_incidents", state=Incidents.incident)
async def search_all_incidents(callback: CallbackQuery, state=FSMContext):
    ...
    await bot.delete_message(callback.message.chat.id, callback.message.message_id)
    from_req.form_story_in_html(db_list=result_search, search_title="Все преступления", userid=callback.from_user.id)
