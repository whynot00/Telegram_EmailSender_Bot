...
@dp.callback_query_handler(text="add_new_incident", state=Incidents.incident)
...
async def insert_fellow_story(message: types.Message, state=FSMContext):
    async with state.proxy() as data:
        data["fellow_story"] = message.text
...
async def insert_revelation_story(message: types.Message, state=FSMContext):
    ...
    if ("да" in message.text.lower() or "нет" in message.text.lower()):
        ...
        await bot.send_message(message.from_user.id, "Введите фабулу преступления:", reply_markup=nav.main_button)
    else:
...
# 1.7 Ввод фотографии
@dp.callback_query_handler(text="add_photo_incident", state=Incidents.incident_add_new_story_5)
...
# 1.8 Подтверждение
@dp.callback_query_handler(text="confirm_add_incident", state=Incidents.incident_add_new_story_5)
async def confirm_add_incident(callback: CallbackQuery, state=FSMContext):
    async with state.proxy() as data:
        result_add = [data["revelation_story"], data["date_story"], data["address_story"], data["fellow_story"], data["story"]]
